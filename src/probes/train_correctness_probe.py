#!/usr/bin/env python
"""
Baseline correctness probe (Zhang-style).

Expected input:
- hidden state tensors (e.g., data/reps/<model>/<dataset>/)
- labels for correctness per chunk

Expected output:
- results/probes/correctness/<run_id>/
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train correctness probe (baseline).")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/probes/correctness"),
...
def main() -> None:
    ...
    raise SystemExit(
        "Not implemented: load reps/labels, train BCE probe, log metrics to "
        f"{args.output_dir}. Align with Zhang et al. baseline."
    )
